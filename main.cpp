...
int main(int argc, char* argv[]) 
{
    if (argc == 2) 
    {
            ...
            for (int i = 0; i < 1; i++)
            {
                for (int j = 0; j < 6; j++)
                {
                    ...
                    for (int k = 0; k < 12; k++)
                    {
                        ...
                        startSorting(algorithms[k], arrCopy1, comparisons1);
                        end1 = high_resolution_clock::now();

                        time1 = duration_cast<milliseconds>(end1 - start1).count();

                        printExperimentsOutput(i, sizes[j], algorithms[k], time1, comparisons1);
        ...
        {   
            cerr << "Usage: <execution file> -e|-a|-c <algorithm1> [algorithm2] <input file(.txt)/input size> [options]" << endl;
            return 1;
        }
    }
